...
#define _alloc_check(fn)                 \
    do {                                 \
        if (!(fn)) {                     \
            _cuda_err = cudaUnspecified; \
            goto _cuda_fail;             \
        } else                           \
            _cuda_err = cudaSuccess;     \
    } while (false)

// Clone CPU <-> CUDA

template <typename T> T* cuda_clone(const void* ptr, int num) {
    T*          cuda_ptr;
    cudaError_t r;
...
template <typename T> T* cpu_clone(const void* ptr, int num) {
    T*          cpu_ptr;
    cudaError_t r;
