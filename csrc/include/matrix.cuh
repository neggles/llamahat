...
class MatrixView_q4_row {
   public:
    ...
    MatrixView_q4_row(const uint32_t* data, const int height, const int width)
        : data(data), height(height), width(width) {}
...
class MatrixView_q4_column {
   public:
    ...
    __device__ __forceinline__
    MatrixView_q4_column(const uint32_t* data, const int height, const int width)
        : data(data), height(height), width(width) {}
...
// Accumulated dot product of 8-element row vectors in h and quantized column vectors in v, constant
// zero/scale
...
__device__ __forceinline__ half2 dot_product_8(
    ...
    for (int i = 0; i < count; i++) {
        ...
        //         half2 v_01 = q4_table[v_zero - 1][(v_read      ) & 0xff]; // (constant memory is
        //         too slow apparently) half2 v_23 = q4_table[v_zero - 1][(v_read >>  8) & 0xff];
