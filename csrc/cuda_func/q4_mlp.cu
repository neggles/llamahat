...
typedef void (*fp_silu_mul_cuda_kernel)(half*, const half*, const int, const int);
...
__global__ void silu_mul_cuda_kernel(
    ...
    if constexpr (use_half2) column *= 2;
...
void q4_mlp_cuda(
    ...
    if (gate_a) {
        half_matmul_cublas_cuda(
            tuningParams, temp_x, gate_a, lora_temp, height, dim, gate_rank, handle
        );
        half_matmul_cublas_cuda(
            tuningParams, lora_temp, gate_b, temp_mlp_0, height, gate_rank, temp_mlp_width, handle
        );
    }
    if (up_a) {
        half_matmul_cublas_cuda(
            tuningParams, temp_x, up_a, lora_temp, height, dim, up_rank, handle
        );
        half_matmul_cublas_cuda(
            tuningParams, lora_temp, up_b, temp_mlp_1, height, up_rank, temp_mlp_width, handle
        );
    }
    ...
    if (!tuningParams->concurrent_streams) {
        ...
        q4_matmul_cuda(
            tuningParams, temp_x, height, gate, buffers->temp_mlp, gate_a ? true : false, str_1
        );
    ...
    // x += temp1 @ down (implicitly add the residual connection by not zeroing the output in the
    // matmul)

    if (down_a) {
        half_matmul_cublas_cuda(
            tuningParams, temp_mlp_0, down_a, lora_temp, height, temp_mlp_width, down_rank, handle
        );
        half_matmul_cublas_cuda(
            tuningParams, lora_temp, down_b, x, height, down_rank, dim, handle, true
        );
    }
