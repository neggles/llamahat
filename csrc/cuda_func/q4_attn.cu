...
__global__ void update_cache_kernel(
    ...
    int state_offset = (z + state_pos[0]) * state_stride[0] + (y + state_pos[1]) * state_stride[1] +
                       (x + state_pos[2]) * state_stride[2];
    int cache_offset = (z + cache_pos[0]) * cache_stride[0] + (y + cache_pos[1]) * cache_stride[1] +
                       (x + cache_pos[2]) * cache_stride[2];
...
void q4_attn_cuda(
    ...
    if (q_a) {
        ...
        half_matmul_cublas_cuda(
            tuningParams, lora_temp, q_b, query_states, q_len, q_rank, dim, handle
        );
    }
    if (k_a) {
        half_matmul_cublas_cuda(tuningParams, temp_x, k_a, lora_temp, q_len, dim, k_rank, handle);
        half_matmul_cublas_cuda(
            tuningParams, lora_temp, k_b, key_states, q_len, k_rank, dim, handle
        );
    }
    if (v_a) {
        half_matmul_cublas_cuda(tuningParams, temp_x, v_a, lora_temp, q_len, dim, v_rank, handle);
        half_matmul_cublas_cuda(
            tuningParams, lora_temp, v_b, value_states, q_len, v_rank, dim, handle
        );
    }
    ...
    if (!tuningParams->concurrent_streams) {
        ...
        rope_cuda(
            tuningParams, key_states, sin, cos, bsz, _rows_per_batch, head_dim, num_heads, past_len
        );
        ...
        // Project q, k, v, add positional embeddings to q, k, update cache tensors with projected
        // k, v
        ...
        q4_matmul_cuda(
            tuningParams, temp_x, q_len, q_proj, query_states, q_a ? true : false, str_1
        );
        rope_cuda(
            tuningParams,
            query_states,
        ...
        q4_matmul_cuda(
            ...
            value_states,
            v_a ? true : false,
            buffers->alt_stream_3
        );
        cudaStreamWaitEvent(str_3, sync_2, 0);
        update_cache_kernel<<<blocks, threads, 0, str_3>>>(
            key_states,
            value_states,
            key_cache,
            value_cache,
...
void q4_attn_2_cuda(
    ...
    if (o_a) {
        ...
        half_matmul_cublas_cuda(
            tuningParams, attn_output, o_a, lora_temp, height, dim, o_rank, handle
        );
