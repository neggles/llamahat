...
#pragma unroll
    for (int k = k0; k < k0 + BLOCKSIZE / 2; k++) {
        ...
        half2 w_item_0 = *w_ptr;
        w_ptr          += w_.width / 2;
        half2 w_item_1 = *w_ptr;
        w_ptr          += w_.width / 2;
    ...
    if constexpr (use_half2 && !odd_rank) {
        ...
        if (threadIdx.y == 0) {
            ...
            for (int i = 1; i < blockDim.y; ++i)
                acc = __hadd(accum[i][threadIdx.x], acc);
...
void half_matmul_small_cuda(
    ...
    dim3 threads(S_THREADS_X, (dim + S_BLOCKSIZE - 1) / S_BLOCKSIZE, 1);

    dim3 blocks((width + S_THREADS_X - 1) / S_THREADS_X, 1, height);
    ...
    if (dim & 0x03) {
        half_matmul_small_kernel<false, true>
            <<<blocks, threads, 0, alt_stream>>>(x, w, out, height, dim, width, no_zero);
    } else {
        if (use_half2)
            half_matmul_small_kernel<true, false>
                <<<blocks, threads, 0, alt_stream>>>(x, w, out, height, dim, width, no_zero);
        else
            half_matmul_small_kernel<false, false>
                <<<blocks, threads, 0, alt_stream>>>(x, w, out, height, dim, width, no_zero);
    }
}
