...
__global__ void q4_matmul_kernel(
    ...
    int w_column = THREADS_X * blockIdx.x +
                   threadIdx.x; // assume width of weight matrix divisible by THREADS_X (32)
    ...
    if constexpr (use_groupsize) {
        // For quant matrices where groupsize divides BLOCK_SIZE_Z we always start on a group
        // boundary, so this could be slightly faster
        ...
        for (int k = x_column, group = x_column / groupsize; k < x_column + iterations * 8;
            ...
            if constexpr (use_half2) {
                ...
                if constexpr (use_x_map)
                    acc = dot_product_8_x_map(
                        acc, x_, x_row, k, w_, k, w_column, w_scale, w_zero, groupsize / 8, x_map
                    );
                else
                    acc = dot_product_8(
                        acc, x_, x_row, k, w_, k, w_column, w_scale, w_zero, groupsize / 8
                    );
            } else {
                ...
                if constexpr (use_x_map)
                    acc_h = dot_product_8_x_map_h(
                        acc_h, x_, x_row, k, w_, k, w_column, w_scale, w_zero, groupsize / 8, x_map
                    );
                else
                    acc_h = dot_product_8_h(
                        acc_h, x_, x_row, k, w_, k, w_column, w_scale, w_zero, groupsize / 8
                    );
            }
        }
    } else {
        // Otherwise assume groupsize is a multiple of 8, do 8 columns per iteration and trust the
        // cache
        ...
        for (int k = x_column; k < x_column + iterations * 8; k += 8) {
            if constexpr (use_half2) {
                ...
                if constexpr (use_x_map)
                    acc = dot_product_8_x_map(
                        acc, x_, x_row, k, w_, k, w_column, w_scale, w_zero, 1, x_map
                    );
                else
                ...
                if constexpr (use_x_map)
                    acc_h = dot_product_8_x_map_h(
                        acc_h, x_, x_row, k, w_, k, w_column, w_scale, w_zero, 1, x_map
                    );
                else
                    acc_h =
                        dot_product_8_h(acc_h, x_, x_row, k, w_, k, w_column, w_scale, w_zero, 1);
            }
        }
    }
...
fp_q4_matmul_kernel q4_matmul_kernel_pick(
...
void q4_matmul_cuda(
    ...
    fp_q4_matmul_kernel kernel =
        q4_matmul_kernel_pick(tuningParams, block_size_z, w->groupsize, x_map);
    kernel<<<blocks, threads, 0, alt_stream>>>(
        x_mapped,
        w->cuda_qweight,
        out,
        w->cuda_scales,
