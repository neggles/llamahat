...
void g_q4_free_matrices() {
    for (const auto& m : g_q4_matrices)
        delete m;
    g_q4_matrices.clear();
}
...
Q4Matrix::Q4Matrix(
    ...
    : height(_height), width(_width), groups(_groups), device(_device) {
...
void Q4Matrix::reconstruct(half* out) {
    ...
    dim3 blocks((width + threads.x - 1) / threads.x, (height / 8 + threads.y - 1) / threads.y, 1);

    reconstruct_kernel<<<blocks, threads>>>(
        cuda_qweight, out, cuda_scales, cuda_qzeros, height / 8, width, groupsize
    );
}
