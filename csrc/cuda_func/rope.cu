...
typedef void (*fp_rope_cuda_kernel)(half*, const half*, const half*, int, int, int, int);
...
__global__ void rope_cuda_kernel(
    ...
    int column = (blockIdx.x * THREADS_X + threadIdx.x);
...
void rope_cuda(
    ...
    kernel<<<blocks, threads, 0, alt_stream>>>(
        x, sin, cos, rows_per_batch, head_dim, num_heads, past_len
    );
}
