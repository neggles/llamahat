...
from llamahat.lora import ExLlamaLora
from llamahat.model import ExLlama, ExLlamaCache
from llamahat.tokenizer import ExLlamaTokenizer
...
class ExLlamaGenerator:
    ...
    sequence_actual: Optional[torch.Tensor]
    settings: Settings
    beams: Optional[int]
    max_beam_length: int
    in_beam_search: True
    disallowed_tokens: Optional[list[int]]
    lora: Optional[ExLlamaLora]

    def __init__(self, model: ExLlama, tokenizer: ExLlamaTokenizer, cache: ExLlamaCache):
        self.model: ExLlama = model
        self.tokenizer: ExLlamaTokenizer = tokenizer
        self.cache: ExLlamaCache = cache
        self.reset()
