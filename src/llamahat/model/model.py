...
from llamahat import cuda_ext

from .settings import ExLlamaConfig
...
class Ex4bitLinear:
    def __init__(self, config, in_features, out_features, has_bias, tensors, key):
        ...
        self.q4 = cuda_ext.make_q4(self.qweight, self.qzeros, self.scales, self.g_idx, self.device_index)
...
class ExLlamaAttention:
    ...
    def fused(self, hidden_states, cache, buffer, input_layernorm, lora):
        ...
        # Output projection
        cuda_ext.q4_attn_2(hidden_states, attn_output, self.o_proj.q4, o_a, o_b, lora_temp)
        # return hidden_states
...
class ExLlama:
    def __init__(self, config: ExLlamaConfig):
        self.config: ExLlamaConfig = config
        ...
        for dev in self.config.device_map.get_layers_devs():
            ...
            cuda_ext.prepare_buffers(torch.device(dev), temp_state, temp_mlp, temp_zeros_float, temp_dq)
